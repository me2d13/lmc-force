...
module dira() {
    zz = 10;
    difference() {
    cylinder(d=5.4, h=zz-1,$fn=100, center=true);
    translate([1.52, -5, -zz/2]) cube([10, 10, zz]);
    translate([-11.52, -5, -zz/2]) cube([10, 10, zz]);
    }
}
...
module male() {
    difference() {
translate([ 0,  0, 0]) rotate([0,0,-($t-n3/4+n1/4+1/2)*360/n3]) color([0.75,0.75,1.00]) gear(mm_per_tooth,n3,thickness,hole);
        dira();
    }
}

module bazmekBezDiry() {
translate([0,  0, 0]) rotate([0,0,-($t-n4/4-n1/4+1/2-floor(n4/4)-2.25)*360/n4]) color([1.00,0.75,0.50]) gear(mm_per_tooth,n4,thickness,0,0,n4-6);
...
translate([-cl+20,  -cw/2, -thickness/2]) cube([cl, cw, thickness]);
    translate([0,  0, 0]) cylinder(d=cw, h=thickness,$fn=100, center=true);
}

module bazmek() {
difference() {
    bazmekBezDiry();
    translate([0,  0, -thickness]) cylinder(d=14.9, h=30,$fn=6, center=true); // sroub M8
    //translate([0,  0, -thickness]) cylinder(d=8, h=30,$fn=100, center=true); 
}
}
